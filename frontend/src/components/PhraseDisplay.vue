...
export default {
  ...
  setup() {
    ...
    // ✅ Watch for changes in isGuessMode and reset UI state when exiting Guess Mode
    watch(() => store.isGuessMode, (newVal) => {
      if (!newVal) {
        store.currentTypedLetter = '';
        store.activeBoxIndex = null;
      }
    });
